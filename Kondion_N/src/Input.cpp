...
namespace Kondion { namespace Input {
	...
	void MouseLock(bool a) {
		glfwSetInputMode(Window::w, GLFW_CURSOR, a ? GLFW_CURSOR_DISABLED : GLFW_CURSOR_NORMAL);
	}

	void Setup() {

		for (unsigned char i = 0; i < 15; i ++) {
			std::cout << "Controller " << int(i) << ": " << (glfwJoystickPresent(i) ? "EXISTS" : "NOPE") << "\n";
		...
		for (unsigned short i = 0; i < controls.size(); i ++) {
			...
			if (controls[i]->device == 0) {
				...
				if (controls[i]->button < CONTROLLER_AXIS) {
					// it is a button
					int dummy;
					controls[i]->x = glfwGetJoystickButtons(joy, &dummy)[controls[i]->button];
				} else { // else if (whatever), but nothing yet
					int dummy;
					controls[i]->x = glfwGetJoystickAxes(joy, &dummy)[controls[i]->button - CONTROLLER_AXIS];
				}
			}
		}
	}
	...
	void DebugPrint() {
		for (unsigned short i = 0; i < controls.size(); i ++) {
			printf("Name: %s Alt: %i X: %4.2f \n",  controls[i]->name.c_str(), controls[i]->alternate, controls[i]->x);
